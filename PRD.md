...
- [x] D0-01 Capture debt baseline report.
...
- [x] D1-01 Eliminate unused imports/vars in `src/main.rs`, `src/rich_editor.rs`, and `src/spec_cli.rs`.
...
- [x] D1-02 Resolve private interface mismatches in UI message/state types.
...
- [x] D1-03 Remove dead code in first-party app paths.
...
- [x] D1-04 Address clippy high-signal patterns in touched first-party files.
...
- [x] D2-01 Extract prompt/approval rendering helpers from `src/main.rs` into `src/ui/prompts.rs`.
  Done when: sandbox/approval prompt rendering no longer lives in `main.rs`.

- [x] D2-02 Extract model metadata/context utilities from `src/main.rs` into `src/model_context.rs` APIs.
  Done when: parameter/context extraction logic is fully outside `main.rs`.

- [x] D2-03 Extract spec tree and plan view composition from `src/main.rs` into `src/spec_ui.rs`.
  Done when: spec rendering helpers are centralized and tested.

- [x] D2-04 Extract message/event parsing/formatting into dedicated module.
...
- [x] D3-01 Add regression tests for each extracted module boundary.
  Done when: targeted tests cover moved logic and pass.

- [x] D3-02 Write debt-pass summary and remaining debt register.
