...
1. **Flexible** 🌀: Multiple quantization formats in _one_ file format with _one_ framework to run them all.
2. **Reliable** 🔒: Compatibility ensured with _embedded_ and _checked_ semantic versioning information from day 1.
3. **Easy** 🤗: Download UQFF models _easily_ and _quickly_ from Hugging Face, or use a local file.
...
> Note: when loading an UQFF model, _any_ ISQ setting will be ignored.
