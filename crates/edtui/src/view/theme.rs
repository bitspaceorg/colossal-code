...
pub struct EditorTheme<'a> {
    ...
    /// The text style for search matches (non-current)
    pub search_match_style: Style,
    /// The text style for the current search match
...
impl Default for EditorTheme<'_> {
    ...
    fn default() -> Self {
        Self {
            ...
            search_match_style: Style::default().bg(BLUE).fg(BLACK),
            search_current_match_style: Style::default().bg(MAGENTA).fg(BLACK),
