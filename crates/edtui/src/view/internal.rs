...
/// A selection with an associated style
pub(crate) struct StyledSelection<'a> {
    pub(crate) selection: &'a Option<Selection>,
...
/// given styled selections. This allows different selections to have different styles.
pub(crate) fn into_spans_with_styled_selections<'a>(
    line: &[char],
    styled_selections: &[StyledSelection],
    row_index: usize,
    col_skips: usize,
    base_style: &Style,
) -> Vec<Span<'a>> {
    ...
    if line.is_empty() {
        let selected_style = styled_selections
            .iter()
            .find(|ss| {
                ss.selection
                    .as_ref()
                    .map_or(false, |sel| sel.contains_row(row_index))
            })
            .map(|ss| ss.style)
    ...
    // When col_skips is beyond line length
    ...
    for (i, &ch) in line.iter().skip(col_skips).enumerate() {
        ...
        // Find the first matching selection (higher priority selections should come first)
        let current_style = styled_selections
            .iter()
            .find(|ss| {
                ss.selection
                    .as_ref()
                    .map_or(false, |sel| sel.contains(&position))
            })
            .map(|ss| ss.style)
            .unwrap_or(base_style);

        // If the style has changed, push the current span and start a new one
        if i != 0 && !std::ptr::eq(previous_style, current_style) {
            spans.push(Span::styled(current_span.clone(), *previous_style));
            current_span.clear();
        }
    ...
    // Push the final span
    spans.push(Span::styled(current_span, *previous_style));
...
pub(crate) fn into_spans_with_selections<'a>(
    ...
    if line.is_empty() {
        ...
        // We check if any selection contains this row at any column position
