...
pub(crate) fn max_col(lines: &Lines, index: &Index2, mode: EditorMode) -> usize {
    match mode {
        EditorMode::Normal => max_col_normal(lines, index),
        EditorMode::Visual => max_col_visual(lines, index),
        _ => max_col_insert(lines, index),
    }
}
...
/// Returns the maximum permissible column value in visual mode.
/// For empty lines, we maintain the same behavior as insert mode to allow
/// proper visual selection behavior.
pub(crate) fn max_col_visual(lines: &Lines, index: &Index2) -> usize {
    if lines.is_empty() {
        return 0;
    }
    lines
