...
pub struct ViewState {
    ...
    /// When wrapping is disabled, this is logical lines.
    /// When wrapping is enabled, this is visual lines.
...
impl ViewState {
    ...
    fn scroll_down(
        ...
        // Check if cursor is already visible by walking forward from viewport.y
        // and seeing if we can fit cursor_row within max_height visual lines
        if cursor_row >= self.viewport.y {
            let mut visual_lines_used = 0;
            let mut last_visible_row = self.viewport.y;

            for row in self.viewport.y..=cursor_row {
                if let Some(line) = lines.get(jagged::index::RowIndex::new(row)) {
                    let line_width = chars_width(line, self.tab_width);
                    let line_height = LineWrapper::determine_split(line_width, max_width).len();

                    // Check if adding this line would exceed available height
                    if visual_lines_used + line_height > max_height {
                        break;
                    }

                    visual_lines_used += line_height;
                    last_visible_row = row;
                }
            }

            // If cursor is within the visible range, don't scroll
            if cursor_row <= last_visible_row {
                return;
            }
        }

        // Cursor is not visible, calculate where viewport should be
        // Walk backwards from cursor, fitting as many lines as possible within max_height
        let mut remaining_height = max_height;
        let mut first_visible_row = 0; // Default to start of document
        ...
        for (i, line) in lines.iter_row().rev().skip(skip).enumerate() {
            ...
            if remaining_height < current_row_height {
                first_visible_row = cursor_row.saturating_sub(i.saturating_sub(1));
                break;
            }
