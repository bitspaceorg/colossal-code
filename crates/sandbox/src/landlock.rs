...
pub fn apply_sandbox_policy_to_current_thread(
    ...
    if !sandbox_policy.has_full_disk_write_access() {
        let writable_roots = sandbox_policy
            ...
            .collect();
        let readable_roots = if !sandbox_policy.has_full_disk_read_access() {
            sandbox_policy.get_readable_roots_with_cwd(cwd)
        } else {
            vec![PathBuf::from("/")]
        };
        install_filesystem_landlock_rules_on_current_thread(writable_roots, readable_roots)?;
    } else if !sandbox_policy.has_full_disk_read_access() {
        // Only read restrictions (no write restrictions)
        let readable_roots = sandbox_policy.get_readable_roots_with_cwd(cwd);
        install_filesystem_landlock_rules_on_current_thread(vec![], readable_roots)?;
    }
...
///
/// Restricts read access to the specified `readable_roots` and write access to
/// `/dev/null` and the provided list of `writable_roots`.
///
/// # Errors
/// Returns [`SandboxError`] variants when the ruleset fails to apply.
fn install_filesystem_landlock_rules_on_current_thread(
    ...
    let access_ro = AccessFs::from_read(abi) | AccessFs::Execute;
    ...
    if status.ruleset == landlock::RulesetStatus::NotEnforced {
        return Err(SandboxError::Sandbox(SandboxErr::LandlockRestrict));
    }
...
fn install_network_seccomp_filter_on_current_thread() -> std::result::Result<(), SandboxError> {
    // Build rule map.
    ...
    let target_arch = if cfg!(target_arch = "x86_64") {
        ...
        return Err(SandboxError::Sandbox(SandboxErr::UnsupportedArchitecture));
    };
