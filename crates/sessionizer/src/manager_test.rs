...
mod tests {
    ...
    fn test_cursor_style_codes() {
        ...
        let output = clean_ansi_codes(input);
        assert_eq!(output, "Hello", "Should remove cursor style codes with space");
    }
    ...
    fn test_cursor_codes_between_chars() {
        ...
        let output = clean_ansi_codes(input);
        assert_eq!(output, "python", "Should remove cursor codes between characters");
    }
    ...
    fn test_realistic_python_output() {
        ...
        let output = clean_ansi_codes(input);
        assert_eq!(output, "python ./numerical_ops.py", "Should clean python command");
    }
