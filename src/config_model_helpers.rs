...
use crate::{
    model_context, persistence, App, ModelInfo, AUTO_SUMMARIZE_THRESHOLD_CONFIG_KEY,
    AUTO_SUMMARIZE_THRESHOLD_VERSION, AUTO_SUMMARIZE_THRESHOLD_VERSION_KEY,
    DEFAULT_AUTO_SUMMARIZE_THRESHOLD, LEGACY_AUTO_SUMMARIZE_THRESHOLD,
    MAX_AUTO_SUMMARIZE_THRESHOLD, MIN_AUTO_SUMMARIZE_THRESHOLD,
};
...
impl App {
    pub(super) fn models_directory_path() -> Option<std::path::PathBuf> {
        dirs::home_dir().map(|home| home.join(".config").join(".nite").join("models"))
    }

    pub(super) fn load_config_value(key: &str) -> Option<String> {
        persistence::config::load_config_value(key)
    }

    pub(super) fn load_vim_mode_setting() -> bool {
        Self::load_config_value("vim-keybind")
            .map(|v| v == "true")
            .unwrap_or(false)
    }

    pub(super) fn load_model_setting() -> Option<String> {
        Self::load_config_value("model")
    }

    pub(super) fn clamp_auto_summarize_threshold(value: f32) -> f32 {
        value.clamp(MIN_AUTO_SUMMARIZE_THRESHOLD, MAX_AUTO_SUMMARIZE_THRESHOLD)
    }

    pub(super) fn load_auto_summarize_threshold_setting() -> f32 {
    ...
    pub(super) fn detect_context_tokens(model: Option<&str>) -> Option<usize> {
        if model.is_none() {
            return None;
        }
    ...
    pub(super) fn refresh_context_window(&mut self) {
    ...
    pub(super) fn save_vim_mode_setting(&self) -> Result<()> {
        self.save_config()
    }

    pub(super) fn load_models(&mut self) -> Result<()> {
    ...
    pub(super) fn initialize_config_file() -> Result<()> {
